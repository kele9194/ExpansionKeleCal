...
{
    public class GaSniperCalF : GaSniperAbs
    {
        public override string LocalizationCategory => "StarySniper";
        // private const string SetNameOverride = "SG幻星元狙击步枪-B";
        const float RightClickCoefficient = 3.2f;
        // 重写基础属性
        public override int BaseDamage => 4059;
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(ModContent.ItemType<GaSniperCalE>(), 1);
...
// 	public class GaSniperCalF : ModItem
// 	{// 右键属性
//          private const int LeftClickDamage = 4059;
//         private const float LeftClickKnockBack = 8f;
//         private const float LeftClickShootSpeed = 32f;
//         private const int LeftClickUseTime = 28;
//         private const int LeftClickUseAnimation = LeftClickUseTime;

//         private const int constcrit = 56;
//         private const int constrare = 10;
//         private const string setNameOverride="SG幻星元狙击步枪F";
...
//         const double rightClickCoefficient = 3.2;
...
//             Item.value = Item.buyPrice(gold: 320); 
//             Item.crit = constcrit;
...
// 	{  
//     // 创建 GaSniperA 武器的合成配方  
//     Recipe recipe = Recipe.Create(ModContent.ItemType<GaSniperCalF>()); 
// 	recipe.AddIngredient(ExpansionKeleCal.calamity.Find<ModItem>("MiracleMatter").Type, 1);
// 	recipe.AddIngredient(ModContent.ItemType<GaSniperCalE>(), 1);
//     recipe.AddTile(ExpansionKeleCal.calamity.Find<ModTile>("DraedonsForge"));//远古操纵机
//     recipe.Register(); // 注册配方  
...
//             // 添加自定义的 tooltip  
//             TooltipLine line = new TooltipLine(Mod, "GaSniperCalFTooltip", "该狙击步枪CalE型号升级版,此后升级均携带超位崩解减益，使用火枪子弹,钨/银子弹时,会转化为高速子弹。");  
//             tooltips.Add(line);  
//         }  
