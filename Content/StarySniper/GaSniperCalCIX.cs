...
namespace ExpansionKeleCal.Content.StarySniper
{
    ...
    public class GaSniperCalCIX : GaSniperAbs
    {
        public override string LocalizationCategory => "StarySniper";
        // private const string SetNameOverride = "SG幻星元狙击步枪-B";
        const float RightClickCoefficient = 2.7f;
        // 重写基础属性
        public override int BaseDamage => 2995;
        public override float KnockBack => 8f;
        public override float ShootSpeed => 32f;
        public override int UseTime => 33;
        public override int Crit => 34;
        public override int Rarity => ExpansionKeleCal.calamity.Find<ModRarity>("Turquoise").Type;
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(ModContent.ItemType<GaSniperCalCI>(), 1);
        ...
        // {
        //     // 添加自定义的 tooltip  
        //     TooltipLine line = new TooltipLine(Mod, "GaSniperCalBTooltip", introduction);
        //     tooltips.Add(line);
        // }
    }
}
	
// 	public class GaSniperCalCIX : ModItem
// 	{// 右键属性
//          private const int LeftClickDamage = 2995;
//         private const float LeftClickKnockBack = 8f;
//         private const float LeftClickShootSpeed = 32f;
//         private const int LeftClickUseTime = 31;
//         private const int LeftClickUseAnimation = LeftClickUseTime;

//         private const int constcrit = 38;
//         private const int constrare = 9;
//         private const string setNameOverride="SG幻星元狙击步枪CIX";
...
//         double critOverflowCoefficient = 2.5;
...
//             // int actualLeftDamage=(int)(LeftClickDamage * (1+damageGenericUp+damageRangedUp));
...
//                 {
//                     //Item.crit=100;
//                     //damage=(int)(actualRightDamage*(1+2.17*(actualCrit*0.01-1)));
//                     damage=(int)(damage*(1+critOverflowCoefficient*(actualCrit*0.01-1)));
//                     //Main.NewText($"Right Click Crit Rate: {rightClickCritRate}%");
//                 //     Main.NewText($"actualCrit: {actualCrit}");
                
//                 // Main.NewText($"Right Click Damage: {damage}");
//                 // Main.NewText($"actualRightDamage: {actualRightDamage}");
//                 }
//                 else{
...
//                 {
//                     //Item.crit=100;
//                     //damage=(int)(actualLeftDamage*(1+2.17*(actualCrit*0.01-1)));
//                     damage=(int)(damage*(1+critOverflowCoefficient*(actualCrit*0.01-1)));
//                     // Main.NewText($"actualCrit: {actualCrit}");
//                     // Main.NewText($"Left Click Damage: {damage}");
//                     // Main.NewText($"actualLeftDamage: {actualLeftDamage}");
//                 }
//                 else{
//                     // Main.NewText($"actualCrit: {actualCrit}");
//                     // Main.NewText($"Left Click Damage: {damage}");
//                     // Main.NewText($"actualLeftDamage: {actualLeftDamage}");
//                 }
//                 // Main.NewText($"DamageClass.Ranged: {player.GetDamage(DamageClass.Ranged).ApplyTo(1)}");
//                 // Main.NewText($"DamageClass.Generic: {player.GetDamage(DamageClass.Generic).ApplyTo(1)}");
//                 //Main.NewText($"Left Click Crit Rate: {leftClickCritRate}%");
                
//             }
//             //Main.NewText($"actualCrit: {actualCrit}");
//             //Main.NewText($"EXactualLeftDamage: {actualLeftDamage}");
//             //Main.NewText($"EXactualRightDamage: {actualRightDamage}");
...
// 	{  
//     // 创建 GaSniperA 武器的合成配方  
//     Recipe recipe = Recipe.Create(ModContent.ItemType<GaSniperCalCIX>()); 
// 	recipe.AddIngredient(ExpansionKeleCal.calamity.Find<ModItem>("UelibloomBar").Type, 7);
//     recipe.AddIngredient(ExpansionKeleCal.calamity.Find<ModItem>("BloodstoneCore").Type, 1);
// 	recipe.AddIngredient(ModContent.ItemType<GaSniperCalCIII>(), 1);
//     recipe.AddTile(TileID.LunarCraftingStation);//远古操纵机
//     recipe.Register(); // 注册配方  
...
// 		public override void ModifyTooltips(System.Collections.Generic.List<TooltipLine> tooltips) {  
//             // 添加自定义的 tooltip  
//             TooltipLine line = new TooltipLine(Mod, "GaSniperCalCIXTooltip", "该狙击步枪CalCIII型号升级版,使用火枪子弹,钨/银子弹时,会转化为高速子弹。");  
//             tooltips.Add(line);  
//         }  
