...
namespace ExpansionKeleCal.Content.StaryArmor
{
	...
	public class StarHelmetCalX : ModItem
	{
		...
		public float a = ArmorData.CalculateA(0.34f+0.3f*ArmorData.GenericDamageBonus[index]/100f+0.2f);
		public static string setNameOverride="幻星元盔X";
    	...
    	return head.type == ModContent.ItemType<StarHelmetCalX>() &&
           body.type == ModContent.ItemType<StarBreastplateCalX>() &&
           legs.type == ModContent.ItemType<StarLeggingsCalX>();
}
    ...
    Recipe recipe = Recipe.Create(ModContent.ItemType<StarHelmetCalX>()); // 替换为 GaSniperA 的类型  
    recipe.AddIngredient(ExpansionKeleCal.calamity.Find<ModItem>("ShadowspecBar").Type, 1);
    recipe.AddIngredient(ModContent.ItemType<StarHelmetCalE>(), 1);
