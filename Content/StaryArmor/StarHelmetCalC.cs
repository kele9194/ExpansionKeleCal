...
namespace ExpansionKeleCal.Content.StaryArmor
{
	...
	public class StarHelmetCalC : ModItem
	{
		...
		public static string setNameOverride="幻星元盔C";
    	...
    	return head.type == ModContent.ItemType<StarHelmetCalC>() &&
           body.type == ModContent.ItemType<StarBreastplateCalC>() &&
           legs.type == ModContent.ItemType<StarLeggingsCalC>();
}
    ...
    Recipe recipe = Recipe.Create(ModContent.ItemType<StarHelmetCalC>()); // 替换为 GaSniperA 的类型  
    recipe.AddIngredient(ExpansionKeleCal.calamity.Find<ModItem>("CosmiliteBar").Type, 2);
    recipe.AddIngredient(ModContent.ItemType<StarHelmetCalB>(), 1);
