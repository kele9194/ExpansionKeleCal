...
namespace ExpansionKeleCal.Content.StaryArmor
{
	...
	public class StarHelmetCalD : ModItem
	{
		...
		public static string setNameOverride="幻星元盔D";
    	...
    	return head.type == ModContent.ItemType<StarHelmetCalD>() &&
           body.type == ModContent.ItemType<StarBreastplateCalD>() &&
           legs.type == ModContent.ItemType<StarLeggingsCalD>();
}
    ...
    Recipe recipe = Recipe.Create(ModContent.ItemType<StarHelmetCalD>()); // 替换为 GaSniperA 的类型  
    recipe.AddIngredient(ExpansionKeleCal.calamity.Find<ModItem>("AuricBar").Type, 2);
    recipe.AddIngredient(ModContent.ItemType<StarHelmetCalC>(), 1);
