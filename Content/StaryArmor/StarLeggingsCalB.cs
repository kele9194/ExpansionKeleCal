...
namespace ExpansionKeleCal.Content.StaryArmor
{
	...
	public class StarLeggingsCalB : ModItem
	{
		...
		public static string setNameOverride="幻星元护胫B";
	...
	{  
	Recipe recipe = Recipe.Create(ModContent.ItemType<StarLeggingsCalB>()); // 替换为 GaSniperA 的类型  
    ...
    recipe.AddIngredient(ExpansionKeleCal.expansionkele.Find<ModItem>("StarLeggingsJ"), 1);
    recipe.AddTile(TileID.LunarCraftingStation);
    recipe.Register(); // 注册配方 
    // 创建 GaSniperA 武器的合成配方  
    Recipe recipeI = Recipe.Create(ModContent.ItemType<StarLeggingsCalB>()); // 替换为 GaSniperA 的类型  
    ...
    recipeI.AddIngredient(ModContent.ItemType<StarLeggingsCalA>(), 1);
